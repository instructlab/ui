...
.PHONY: teardown
teardown: check-kind ## Teardown the kind cluster
	$(CMD_PREFIX) kind delete cluster --name $(ILAB_KUBE_CLUSTER_NAME)

.PHONY: setup-kind
setup-kind: check-kind check-kubectl teardown ## Create a kind cluster with ingress enabled
...
deploy: wait-for-readiness ## Deploy a InstructLab UI development stack onto a kubernetes cluster
	...
	$(CMD_PREFIX) cp .env ./deploy/k8s/overlays/kind/.env
...
.PHONY: run-dev-on-kind ## Run the development environment on kind
run-dev-on-kind: setup-kind deploy ## Setup a kind cluster and deploy InstructLab UI on it
...
deploy-openshift: ## Deploy the InstructLab UI on OpenShift
	...
	$(CMD_PREFIX) cp .env ./deploy/k8s/overlays/openshift/.env
