...
type ILabServer struct {
  ...
  // Base model reference (used in some logic but not necessary if VLLM is used with default paths)
  baseModel string
...
func (srv *ILabServer) runServer(cmd *cobra.Command, args []string) {
  ...
  if srv.debugEnabled {
    srv.log.Info("Debug logging is ENABLED.")
  } else {
    srv.log.Info("Debug logging is DISABLED.")
  }
