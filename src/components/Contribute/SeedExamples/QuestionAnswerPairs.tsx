...
const QuestionAnswerPairs: React.FC<Props> = ({
  ...
  return (
    <Flex style={{ padding: MdSpacerSize.var }} direction={{ default: 'column' }} gap={{ default: 'gapMd' }}>
      ...
      <FlexItem>
        <Form>
          <FormGroup id="" label="Context" isRequired={!isSkillContribution}>
            <TextArea
              isRequired
              type="text"
              aria-label={`Context ${seedExampleIndex + 1}`}
              placeholder={`Enter the context from which the Q&A ${isSkillContribution ? 'pair is' : 'pairs are'} derived. ${isSkillContribution ? '(optional)' : '(500 words max)'}`}
              value={seedExample.context}
              validated={isSkillContribution ? ValidatedOptions.default : seedExample.isContextValid}
            ...
            {!isSkillContribution && seedExample.isContextValid === ValidatedOptions.error && (
              <FormHelperText>
                <HelperText>
          ...
          {seedExample.questionAndAnswers.map((questionAndAnswerPair: QuestionAndAnswerPair, questionAnswerIndex) => (
            <FormGroup
              ...
              label={`Q&A Pair${isSkillContribution ? '' : ` ${questionAnswerIndex + 1}`}`}
