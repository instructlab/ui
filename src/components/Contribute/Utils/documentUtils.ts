...
export const UploadKnowledgeDocuments = async (
  ...
  if (knowledgeFormData.filesToUpload.length > 0) {
    ...
    const fileContents: { fileName: string; fileContent: string; action: string }[] = [];
        ...
        (file) =>
          new Promise<void>((resolve, reject) => {
            ...
            reader.onload = (e) => {
              ...
              fileContents.push({ fileName: file.name, fileContent: fileContent, action: 'add' });
              resolve();
            };
    ...
    if (fileContents.length === knowledgeFormData.filesToUpload.length) {
      try {
