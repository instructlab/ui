...
const getTableColumnSortByIndex =
  <T>({ columns, subColumns, sortIndex, sortDirection, setSortIndex, setSortDirection }: TableColumnSortByIndexProps<T>): GetColumnSort =>
  (columnIndex: number) =>
    (columnIndex < columns.length ? columns[columnIndex] : subColumns?.[columnIndex - columns.length])?.sortable
      ? {
          sortBy: {
            index: sortIndex,
...
const useTableColumnSort = <T>(
  ...
  const [activeSortIndex, setActiveSortIndex] = React.useState<number | undefined>(defaultSortColIndex);
  const [activeSortDirection, setActiveSortDirection] = React.useState<'desc' | 'asc' | undefined>(defaultSortDirection);
  ...
  return {
    transformData: (data: T[]): T[] => {
      ...
      return data.toSorted((a, b) => {
        const columnField = activeSortIndex < columns.length ? columns[activeSortIndex] : subColumns[activeSortIndex - columns.length];
