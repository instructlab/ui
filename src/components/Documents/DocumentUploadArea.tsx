...
const DocumentUploadArea: React.FunctionComponent<UploadFileProps> = ({ existingFiles, onUploaded, addAlert }) => {
  ...
  const isConverting = React.useRef<boolean>(false);
  ...
  const overwriteFiles = async () => {
    ...
    setFilesToConvert(droppedFiles);
    setDroppedFiles([]);
  };
  ...
  const onError = React.useCallback((errorMessage: string) => addAlert(errorMessage, 'danger'), []);

  const onFilesConverted = React.useCallback(
    async (convertedFiles: File[]) => {
      setFilesToConvert([]);
  ...
  const onConversionCanceled = React.useCallback(() => {
    setFilesToConvert([]);
  ...
  React.useEffect(() => {
    const convertFiles = async () => {
      if (filesToConvert.length) {
        ...
        const convertedFiles = await convertFilesToMarkdown(filesToConvert, () => isConverting.current, onError);
        if (convertedFiles.length) {
          await onFilesConverted(convertedFiles);
        }
      }
    };
    convertFiles();
