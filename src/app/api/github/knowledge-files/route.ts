...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { branchName, currentCommitSHA, files } = body;
    ...
    const finalFiles = new Map<string, { fileName: string; fileContent: string; action: string }>();
    ...
    files.map((file: { fileName: string; fileContent: string; action: string }) => {
      if (file.action == 'delete') {
        console.log(`Deleting existing file ${file.fileName} from knowledge contribution ${branchName}`);
        finalFiles.delete(file.fileName);
      } else {
...
async function createFilesCommit(
  ...
  subDirectory: string,
  files: Map<string, { fileName: string; fileContent: string; action: string }>,
  userEmail: string,
  baseSha: string
): Promise<string> {
